...
- `src/ship-defense/engagement-runtime.ts`: shared observed-target planning for ship defense. It consumes standard `DefenseObservation` inputs through `DefenseConsumer`, applies the domain threat score and engagement ledger policy, filters real magazine/envelope availability, and selects a weapon before transactional physical-launcher reservation. The scene layer executes the returned plan; it does not rank targets or choose a weapon independently.
