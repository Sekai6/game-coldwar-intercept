...
- `src/air/scenarios.ts`: scenario-owned `joint`, `intercept`, and `strike` presets with aircraft formations, spawn geometry, mission overrides, and protected-formation relationships.
...
- `src/ship-defense/launcher-runtime.ts`: common physical-launcher resource reservation (weapon compatibility, alternating mounts/banks, readiness and Mk 41 deck separation); Mk 10 task, slew, physical rail release, return-to-load, reload, casualty cancellation, and reset lifecycle; Mk 41 hatch opening, bank sequencing, physical vertical release, spent-cell closure, retained-round cancellation, damage isolation, trapped-round casualty handling, reset, and diagnostic snapshots. The scene layer injects tracks, health, ammunition, authorization settlement, interceptor creation, and visual effects rather than owning launcher selection or phase transitions.
- `src/ship-defense/engagement-runtime.ts`: shared observed-target planning for ship defense. It consumes standard `DefenseObservation` inputs through `DefenseConsumer`, applies the domain threat score and engagement ledger policy, filters real magazine/envelope availability, and selects a weapon before transactional physical-launcher reservation. It also scales terminal illuminator capacity by installed directors and subsystem health, preserves valid assignments, prevents duplicate illumination of one target, releases invalid assignments, and chooses the free director with the least slew. The scene layer executes plans and renders director motion/beams; it does not rank targets, choose weapons, or allocate fire-control resources independently.
- `src/aar/acmi-exporter.ts`: pure AAR-to-Tacview ACMI 2.2 serializer. It pre-scans stable game IDs into Tacview numeric aliases for valid parent/target relations, converts local east/north/altitude coordinates around a virtual reference point, and emits physical-velocity attitude, speed, vertical speed, health and state telemetry. Weapon creation emits `HasFired`; explicit terminal state emits one `Destroyed` event and deletion, while ordinary disappearance is deletion-only. Render-model Euler corrections never enter AAR kinematics. Browser downloading is isolated in `src/aar/download.ts`.
...
Air mission selection adapts `AirTrack` to `DefenseObservation` and lets the shared consumer plus aircraft engagement ledger exclude committed targets. `AirCombatSystem` does not repeat that policy with a truth-side target gate. Successful authorization reserves a compatible mounted hardpoint; the weapon instance is created and ammunition consumed only at physical separation, then the shared record is settled on hit, miss, target loss, or cancellation.
...
9. Air weapons reserve a compatible catalog hardpoint and the required datalink/illumination channel before release. The mounted weapon remains parented to the aircraft until its release clock expires, then preserves its world transform, receives ejector velocity, waits for motor ignition, and only then enters guidance.
10. Air-to-air seeker and illumination eligibility lives in `src/air/guidance.ts`. The runtime supplies sensor geometry and signatures; AI defensive maneuver receives only `AirTrack` warning estimates. After capture, `seekerMeasurementPoint()` converts target truth into a deterministic noisy measurement before aim-point guidance; exact truth remains limited to measurement generation and physical hit resolution.
11. Countermeasure programs enqueue timed physical releases rather than applying a burst modifier in one frame. `src/air/flight-dynamics.ts` is the pure flight-envelope core; rendering consumes its bounded bank, pitch, speed, stall, and per-tick fuel-burn outputs. `consumeFuel()` owns non-negative fuel integration so runtime cannot accidentally apply or cancel `dt` twice.
12. Formation slot geometry and hysteretic join/break/rejoin state live in `src/air/formation.ts`; damage disposition, model-local hit-zone resolution, and loss-of-control descent integration live in `src/air/damage.ts`. Nose, engine bay, outer-wing, tail-control, and fuselage hits select systems from geometry rather than a random subsystem draw. Runtime applies the pure loss-of-control position, velocity, roll, and sea-impact result instead of duplicating those constants.
13. Aircraft models own reusable smoke, fire, and impact-splash visual nodes. Runtime drives their severity from observable damage state and keeps mission-killed aircraft present through the full loss-of-control and sea-impact chain.
14. `src/air/launch-management.ts` owns fire-control usage and weapon eligibility; `src/air/countermeasure-program.ts` owns release scheduling and inventory transitions. `AirCombatSystem` orchestrates their results and creates physical objects but does not duplicate resource thresholds.
15. `src/air/ooda.ts` owns mission-track selection, no-contact mission direction, warning-track defensive geometry, and mission-completion decisions. It accepts observations, the aircraft's own state, and counts rather than hostile world entities. `AirSpawn.mission` and `protectedFormationId` declare escort relationships in scenario data; runtime resolves the protected formation leader without aircraft-ID branches. Escort aircraft use that friendly entity as the threat-priority origin and reuse the formation slot/rejoin state machine for screen geometry. A strike aircraft without a valid surface track continues its prebriefed heading or formation slot; it cannot turn toward a ship's truth transform.
16. `src/air/track-store.ts` owns measurement construction, classification, continuous dead-reckoning, uncertainty growth, quality decay, and expiry. Scan code supplies measurements but never performs a second implicit propagation step.
17. `src/air/missile-runtime.ts` owns air-to-air phase transitions, pre/post-capture guidance-point selection, and target-loss endurance. Midcourse phase changes use the command track rather than target truth, and a lost entity no longer deletes the weapon immediately.
18. `verify:air-intercept` runs a generic no-countermeasure verification preset through `?airCountermeasures=off`. It must observe Phoenix active-seeker acquisition followed by a physical hit on a Tu-16K. The default joint scenario keeps ECM and physical chaff enabled and separately proves the contested seeker path.
19. `CombatPicture` accepts stable numeric or string source references. Air-defense contacts retain their real air-entity IDs and are no longer inserted into the legacy `missiles[]` collection merely to obtain an array index. Launcher queues, illuminators, deterministic guidance seeds, CIWS, AAR, and diagnostics resolve targets through the shared defense-target registry.
20. `DefenseTarget` is the minimal target contract for shipboard interceptors, launcher requests, illuminators, CIWS, and engagement accounting. Legacy `Missile` extends it with threat-runtime fields, while air contacts carry none of the legacy age/history/path/launch/attitude state.
21. Air-defense records reference the real `CombatEntity` through `DefenseTarget.entity`; identity, side, category, position, velocity, signatures, and alive state are no longer duplicated through `externalAir*` fields. `AirShipDefenseContact` transports the entity itself plus only ship-defense presentation/template data.
22. `DefenseTarget.threatType` names the game-scaled threat/weapon profile. `CombatEntity.kind` is reserved for the physical entity category (`ship`, `aircraft`, `missile`, or `decoy`); AAR snapshots and all ship-defense guidance/CIWS lookups follow the same naming split.
23. Aircraft and air-launched weapons implement `TargetableEntity.applyDamage`. Shipboard SAM resolution invokes that one entity contract and reads the resulting `alive` state; it no longer calls aircraft-specific damage or missile-destruction APIs. Aircraft callbacks retain subsystem damage/disposition, while weapon callbacks terminate the physical weapon entity.
   `verify:aircraft-sam-damage` runs the intercept preset until a physical Mk 10/Mk 41 shot produces a Tu-16K model-local subsystem event, proving that aircraft targeting is more than a launch-queue registration.
24. Air-to-air and air-to-ship proximity resolution uses the same `TargetableEntity.applyDamage` contract. Weapon runtime handles only physical fuze geometry and its own termination; target-specific subsystem, hull, or weapon-death transitions remain inside the target callback. Decoys remain non-targetable signal entities and are handled as seeker captures.
25. `AirCombatSystem.updateMissile()` is an orchestrator rather than a monolithic guidance implementation. Release coast, target-loss continuation, datalink refresh, decoy filtering, seeker capture, semi-active illumination, aim competition, kinematic integration, and fuze resolution are isolated stage methods; anti-ship guidance remains on its shared dedicated path.
9. `src/anti-ship-guidance.ts` owns the side-neutral boost/midcourse/terminal transition, command-track guidance, probabilistic active-seeker FOV capture, altitude, speed, and turn envelope used by air-launched AGM-84A/KSR-5. Capture probability is supplied from measured target RCS, ECM, burn-through state, and an explicit low-terminal-altitude sea-clutter factor; their envelopes are catalog data, not weapon-ID branches.
...
Incoming-weapon priority is also observed-state only. `platforms/defense.ts` adapts each incoming track to a stable-ID `DefenseObservation`, then uses the shared `DefenseConsumer` ranking and platform-owned `EngagementRecord` ledger used by aircraft and friendly ship defense. It estimates closing speed, time to impact, range urgency, and local saturation from valid platform tracks, then produces one deterministic ordering. Platform defensive-beam maneuvering and point-defense channel allocation consume the same ordering. Tracks that have exhausted their platform-configured engagement limit leave the point-defense queue, while unobserved truth contacts contribute neither priority nor saturation penalty. The same module maps point-defense subsystem health into effective channel count, reaction delay, and channel-cycle delay; damage therefore changes scheduling as well as PK, while health at or below five percent produces one offline event and no fire. Point-defense fire is authorized transactionally: finite inventory and a physical channel are reserved before the engagement is recorded, and failed authorization cannot pollute the ledger. It is not an instantaneous kill roll: range and the platform-configured effective effector speed produce a time of flight, the channel remains occupied through arrival, and the incoming missile continues moving. A shot that arrives after impact has no effect. Each platform also owns a finite definition-sized inventory of effective point-defense engagement bursts. Inventory is consumed at fire time, in-flight rounds still resolve after the final burst is committed, and the first later eligible request emits one depletion event without deleting its track. Hit and miss settle the shared ledger; soft kill, impact, and terminal miss cancel any pending engagement. A point-defense miss is explicitly non-terminal: the incoming missile and track remain alive, then observe a separate platform-configured re-engagement delay before any free channel may accept that track again. A later shoot-look-re-engage attempt uses a separate deterministic draw. Only a resolved kill, soft kill, impact, or terminal miss removes the incoming track.
