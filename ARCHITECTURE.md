...
15. `src/air/ooda.ts` owns mission-track selection, no-contact mission direction, warning-track defensive geometry, and mission-completion decisions. It accepts observations, the aircraft's own state, and counts rather than hostile world entities. Escort aircraft reference a friendly protected entity, use its position as the threat-priority origin, and reuse the formation slot/rejoin state machine for screen geometry. A strike aircraft without a valid surface track continues its prebriefed heading or formation slot; it cannot turn toward a ship's truth transform.
