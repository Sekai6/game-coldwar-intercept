...
12. Formation slot geometry and hysteretic join/break/rejoin state live in `src/air/formation.ts`; damage disposition lives in `src/air/damage.ts`. Runtime applies those decisions but does not duplicate their thresholds.
...
15. `src/air/ooda.ts` owns mission-track selection, warning-track defensive geometry, and mission-completion decisions. It accepts observations and counts rather than world entities, keeping target truth outside AI policy.
