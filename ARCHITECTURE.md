...
- `src/aar/acmi-exporter.ts`: pure AAR-to-Tacview ACMI 2.2 serializer. It pre-scans stable game IDs into Tacview numeric aliases for valid parent/target relations, converts local east/north/altitude coordinates around a virtual reference point, emits three-axis attitude, speed, vertical speed, health and state telemetry, creates/removes objects with destruction events, and carries AAR events into the Tacview timeline. Browser downloading is isolated in `src/aar/download.ts`.
