...
12. Formation slot geometry and hysteretic join/break/rejoin state live in `src/air/formation.ts`; damage disposition and model-local hit-zone resolution live in `src/air/damage.ts`. Nose, engine bay, outer-wing, tail-control, and fuselage hits select systems from geometry rather than a random subsystem draw. Runtime applies those decisions but does not duplicate their thresholds.
