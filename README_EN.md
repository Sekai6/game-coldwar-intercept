...
Both selectable ships declare a data-driven `surfaceStrike` capability and eight physical Mk 141 hardpoints. A separate surface picture tracks the enemy platform with refresh, uncertainty, RCS, sensor health, and quality effects; a correlation-ID change or quality drop resets continuity. CG-57 requires 2.2 seconds of confirmation plus a 1.6-second fire-control delay, while CGN-9 requires 2.8 plus 2.2 seconds. Auto doctrine then launches four-round salvos; `SURFACE STRIKE` hold and `LAUNCH HARPOON` provide manual control.
...
The second CG-57 precision pass stretches the hull and every first-level equipment position together without applying destructive nonuniform scale to circular gun mounts, radar faces, or mast sections. It adds bridge wings, twin Mk 141 quad Harpoon launchers, AN/SLQ-32 side arrays, recessed boat bays and davits, life-raft canisters, hawse holes, hull plating seams, and a helicopter-deck RAST guide. Subsystem hit points, damage zones, and both VLS launch origins use the same calibrated longitudinal coordinates.
