...
Mission-level OODA policy lives in `src/air/ooda.ts`: CAP, intercept, and anti-ship selection consume only track classification and estimated position, defensive beaming and TTI consume only missile-warning tracks, and CAP/intercept return checks both hostile aircraft and hostile weapons still in flight. Without a valid surface track, a strike aircraft holds its prebriefed heading or formation slot instead of reading the ship truth transform. `verify:air-ooda` and `verify:air-resources` are first-class npm scripts.
