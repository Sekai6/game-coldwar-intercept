...
The scenario panel's `AIR PRESET` offers `JOINT`, `INTERCEPT`, `STRIKE`, and `FIGHTER`. Joint adds two-ship F-14A, Tu-16K, and A-6E formations; intercept keeps F-14A/Tu-16K; strike keeps A-6E; fighter pits an F-14A CAP against a two-ship MiG-29A intercept. Formation composition, coordinates, mission overrides, and escort relationships live in `src/air/scenarios.ts`; core runtime receives only `AirSpawn[]`. Fuel consumes the dynamics core's per-tick burn exactly once, and `consumeFuel` prevents negative inventory or duplicate timestep application.
...
The red `MiG-29A Fulcrum-A (9.12)` carries a game-scaled N019 Rubin radar, four semi-active-radar R-27R Alamo-A missiles, and two infrared R-73 Archer missiles. All six weapons occupy physical hardpoints and pass compatibility, fire-control-channel, release-delay, separation, and ignition takeover; R-27R requires continuing illumination, while R-73 competes against target aspect and physical flares. Its procedural model includes the twin-engine nacelles, twin canted tails, leading-edge root extensions, separate intakes, and representative proportions. It uses the same formation, fuel, ECM, countermeasure, subsystem-damage, ship-SAM, and air-to-air engagement contracts as every other aircraft.

CAP/intercept AI now holds contact loss for 20 seconds instead of permanently returning after one radar-refresh or ECM track dropout. An aircraft with a live semi-active weapon balances its defensive turn against the illumination direction; R-27R still loses illumination after the grace period when the target actually leaves the radar sector or its track becomes stale. Air combat uses shoot-look-shoot: a live weapon blocks duplicate shots, then a two-second BDA delay permits a follow-up only if the target remains alive with a valid track.

The four procedural aircraft now have distinct recognizable silhouettes: F-14A wing gloves, variable-sweep panels, twin nacelles and tails; Tu-16K glazed nose, strongly swept wings, wing engine pods and ventral radar; A-6E side-by-side canopy, blunt radome, shoulder intakes and dorsal speed brake; and MiG-29A LERX blending, separated intakes and canted twin tails. Flight visuals include exhaust scaled by speed and engine health, restrained high-speed/high-bank condensation vortices, multi-strip chaff, and clustered flare particles. Every effect follows a bounded entity lifetime rather than unbounded per-frame particle allocation.
