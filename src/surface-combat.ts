...
export type SurfaceStrikeEvent =
  | { kind: "seeker"; missile: SurfaceStrikeMissile }
...
export function updateSurfaceStrikeMissile(
  ...
  const range = Math.min(commandRange, trueRange);
  const terminal = range < 130;
  ...
  if (terminal && !missile.seekerLogged) {
    ...
    missile.mesh.userData.seekerState = "ACTIVE";
    return { kind: "seeker", missile } satisfies SurfaceStrikeEvent;
  }
