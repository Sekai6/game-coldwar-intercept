...
export function createCinematicAtmospherePass() {
  ...
  const pass = new ShaderPass({
    ...
    fragmentShader: `
      precision highp float;varying vec2 vUv;uniform sampler2D tDiffuse;uniform sampler2D cinematicLut;uniform sampler2D ultraScatter;uniform vec2 sunPosition;uniform float godRayStrength;uniform float chromaticAberration;uniform vec2 resolution;uniform float lutSize;uniform float ultraScatterMix;uniform float ultraTime;
      ...
      void main(){vec3 base=sampleChromatic(vUv);vec2 delta=(sunPosition-vUv)/28.;vec2 uv=vUv;float illumination=0.;float weight=0.;float decay=1.;
        ...
        vec3 rayColor=vec3(1.,.76,.48)*rays;float horizonBand=exp(-pow((vUv.y-.47)*7.5,2.));float ultraFog=scatter.r*(1.-scatter.b*.28)*ultraScatterMix*.052;vec3 aerialPerspective=vec3(.43,.56,.64)*(horizonBand*.045+ultraFog);vec3 color=sampleLut(base+rayColor+aerialPerspective);
