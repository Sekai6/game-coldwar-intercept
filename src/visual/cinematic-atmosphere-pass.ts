  ...
  const pass = new ShaderPass({
    ...
    fragmentShader: `
      ...
      void main(){vec3 base=sampleChromatic(vUv);vec2 delta=(sunPosition-vUv)/28.;vec2 uv=vUv;float illumination=0.;float weight=0.;float decay=1.;
        ...
        vec3 rayColor=vec3(1.,.76,.48)*rays;float horizonBand=exp(-pow((vUv.y-.47)*7.5,2.));float sceneDepth=texture2D(tDepth,vUv).x;float distance=viewDistance(sceneDepth);float geometryMask=1.-smoothstep(.9985,1.,sceneDepth);float distanceFog=smoothstep(120.,650.,min(distance,900.))*geometryMask*ultraScatterMix;float scatterModulation=.78+scatter.r*.22;float ultraFog=distanceFog*scatter.r*(1.-scatter.b*.28)*.02;vec4 froxel=integrateFroxel(vUv,min(distance,900.));float froxelRange=smoothstep(5.,220.,distance)*geometryMask;vec3 fogColor=vec3(.43,.56,.64);vec3 atmospheric=mix(base+rayColor,fogColor,distanceFog*scatterModulation*(.28+scatter.b*.14));vec3 froxelResolved=atmospheric*exp(-froxel.a*.68)+froxel.rgb*.72;atmospheric=mix(atmospheric,froxelResolved,froxelMix*froxelRange);vec3 aerialPerspective=fogColor*(horizonBand*.045+ultraFog);vec3 color=sampleLut(atmospheric+aerialPerspective);
