  ...
  constructor() {
    ...
    this.highQualityMaterial = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main(){vec2 p=vec2(position.x+modelMatrix[3].x,modelMatrix[3].z-position.y);vec3 tangent=vec3(1,0,0),binormal=vec3(0,0,1),offset=vec3(0);
          ...
          offset*=mix(1.,.34,ultraSpectrumMix);tangent=mix(tangent,vec3(1,0,0),.66*ultraSpectrumMix);binormal=mix(binormal,vec3(0,0,1),.66*ultraSpectrumMix);float frameTime=mod(time*.7,spectrumFrames);float frame0=floor(frameTime),frame1=ceil(frameTime),frameBlend=fract(frameTime);vec4 spectral=mix(spectrum(p,frame0),spectrum(p,frame1),frameBlend);vec4 spectralX=mix(spectrum(p+vec2(3.,0.),frame0),spectrum(p+vec2(3.,0.),frame1),frameBlend);vec4 spectralZ=mix(spectrum(p+vec2(0.,3.),frame0),spectrum(p+vec2(0.,3.),frame1),frameBlend);vec2 relative=p-wakePosition;float longitudinal=dot(relative,wakeDirection);float lateral=dot(relative,vec2(-wakeDirection.y,wakeDirection.x));float trail=step(longitudinal,0.)*exp(longitudinal*.009);float armWidth=max(3.0,-longitudinal*.22);float kelvinArm=trail*smoothstep(1.7,0.,abs(abs(lateral)-armWidth));float turbulent=trail*exp(-abs(lateral)/max(1.6,-longitudinal*.018));float bow=exp(-dot(relative,relative)*.022);float wakeHeight=(sin(longitudinal*.58+abs(lateral)*.74)*kelvinArm*.22+sin(longitudinal*.9)*turbulent*.07+bow*.18)*wakeStrength;float splashHeight=0.,splashFoam=0.;for(int i=0;i<8;i++){float age=time-splashes[i].z;float energy=splashes[i].w;float radius=age*(5.+energy*2.);float d=length(p-splashes[i].xy);float alive=step(0.,age)*step(age,5.);float ring=exp(-pow((d-radius)/max(.7,.8+age*.28),2.))*alive*energy*exp(-age*.42);splashHeight+=ring*sin((d-radius)*1.8)*.42;splashFoam+=ring;}vec3 fftOffset=vec3(spectral.r*2.-1.,spectral.b*2.-1.,spectral.g*2.-1.);offset+=fftOffset*vec3(1.35,.82,1.35)*ultraSpectrumMix;offset.y+=wakeHeight+splashHeight;tangent.y+=(spectralX.b-spectral.b)*.61*ultraSpectrumMix;binormal.y+=(spectralZ.b-spectral.b)*.61*ultraSpectrumMix;vJacobianFoam=max(spectral.a*ultraSpectrumMix,clamp(splashFoam*.68,0.,1.));vWakeFoam=clamp((kelvinArm*.42+turbulent*.1)*wakeStrength+bow*wakeStrength*.28,0.,1.);
