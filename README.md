...
任务级 OODA 规则位于 `src/air/ooda.ts`：CAP/拦截/护航/反舰选敌只使用航迹分类与估计位置，防御横切和命中倒计时只使用导弹告警航迹。第二架 F-14A 作为 A-6E 长机的护航机，以受保护平台为威胁排序原点，并复用编队槽位与失散重集合状态；攻击机释放后护航任务仍保持。没有有效水面航迹时，攻击机只保持预定任务航向或编队槽位，不读取敌舰真值位置。
