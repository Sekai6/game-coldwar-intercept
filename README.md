...
2. 初始阶段通常只有 ESM 方位提示，不能立即形成武器火控解算。
3. 双方 OODA 会依据带误差的被动航迹选择接近。
4. 进入有机雷达地平线并建立稳定航迹后，双方才能规划 P-500 或 Harpoon 齐射。
5. 使用 `TIME` 切换到 2X 或 4X 可加快超视距接近阶段。
6. 使用数字键 `5` 查看敌平台近景，数字键 `2` 返回默认战术视角。

默认 65 km 是平台定义中的战术距离，不是敌舰的绝对 Z 坐标。场景面板使用世界坐标，`10` 个世界单位等于 `1 km`；默认己方 `Z=40`、敌方 `Z=-610`，实际间隔正好为 65 km。
...
敌方雷达辐射时，双方电子支援系统可生成质量约 `0.12-0.20`、距离误差很大的被动方位航迹。ESM 只允许 OODA 接近，不能累计武器火控时间。
