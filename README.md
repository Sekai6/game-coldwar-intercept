...
场景面板的 `AIR PRESET` 提供 `JOINT`、`INTERCEPT` 与 `STRIKE`：联合场景加入 F-14A、Tu-16K 与 A-6E 三个双机编队，拦截场景只保留 F-14A/Tu-16K，反舰场景只保留 A-6E。编组、坐标、任务覆盖和护航关系位于 `src/air/scenarios.ts`，核心运行时只接收 `AirSpawn[]`。燃油按动力学返回的单 tick 消耗量扣减，`consumeFuel` 保证不会重复应用时间步或出现负值。
...
增强遥测包含三轴姿态、速度、垂直速度、平台健康、任务/制导阶段、失能状态、诱饵阵营、发射平台和目标关系。`Parent` 与 `Target` 会预扫描整局实体并转换为 Tacview 数字对象 ID；对象离开 AAR 时同时写入销毁事件和删除记录。
