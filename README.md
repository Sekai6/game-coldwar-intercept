...
场景面板的 `AIR PRESET` 提供 `JOINT`、`INTERCEPT` 与 `STRIKE`：联合场景加入 F-14A、Tu-16K 与 A-6E 三个双机编队，拦截场景只保留 F-14A/Tu-16K，反舰场景只保留 A-6E。编组、坐标、任务覆盖和护航关系位于 `src/air/scenarios.ts`，核心运行时只接收 `AirSpawn[]`。每架飞机均为独立实体，拥有三维受限机动、编队跟随、带误差雷达航迹、燃油、ECM、箔条/热焰弹以及机体、发动机、雷达、飞控和武器系统毁伤。
...
飞机武器现在必须经过目录声明的实体挂点：兼容性检查、火控通道预约、释放延迟、保持世界姿态离架、抛射下沉和延迟点火后才进入制导。弹药在物理离架时扣减，攻击机也只在成功释放后转入脱离。`npm run verify:air-hardpoints` 验证授权、挂点清空、离架和点火接管顺序。
