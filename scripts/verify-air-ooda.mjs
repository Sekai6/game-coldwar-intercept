...
const result={cap:cap?.targetId,strike:strike?.targetId,defense,strikeNoContact,returnClear:missionShouldReturn({mission:"cap",hasEngaged:true,observedHostileAircraft:0,observedThreats:0}),interceptReturn:missionShouldReturn({mission:"intercept",hasEngaged:true,observedHostileAircraft:0,observedThreats:0}),returnDenied:missionShouldReturn({mission:"cap",hasEngaged:true,observedHostileAircraft:0,observedThreats:1}),returnBeforeContact:missionShouldReturn({mission:"cap",hasEngaged:false,observedHostileAircraft:0,observedThreats:0})};
console.log(JSON.stringify(result,null,2));
if(result.cap!=="near-air"||result.strike!=="ship"||Math.abs(defense.timeToImpact-5)>.001||Math.abs(defense.direction.z+1)>.001||strikeNoContact.x!==-.2||strikeNoContact.y!==-.01||strikeNoContact.z!==.98||!result.returnClear||!result.interceptReturn||result.returnDenied||result.returnBeforeContact)process.exitCode=1;
