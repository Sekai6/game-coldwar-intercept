...
try {
  ...
  await page.waitForFunction(() => {
    ...
    return launches.includes("AIM-54A") && launches.includes("KSR-5") && launches.includes("AGM-84A") && launches.includes("SHIP SAM");
  }, null, { timeout: 25_000 });
  await page.keyboard.press("6");
  await page.waitForTimeout(500);
  const result = await page.locator("#scene").evaluate(c => ({ enabled:c.dataset.airCombatEnabled,total:Number(c.dataset.aircraftTotal??0),live:Number(c.dataset.aircraftLive??0),blueLive:Number(c.dataset.aircraftBlueLive??0),redLive:Number(c.dataset.aircraftRedLive??0),launches:Number(c.dataset.airWeaponsLaunched??0),launchLog:c.dataset.airWeaponLaunchLog,activeWeapons:Number(c.dataset.airWeaponsActive??0),shipSamShots:Number(c.dataset.shipSamShots??0),chaff:Number(c.dataset.airChaff??0),flares:Number(c.dataset.airFlares??0),hits:Number(c.dataset.airCombatHits??0),states:c.dataset.aircraftStates,phases:c.dataset.airWeaponPhases }));
  result.errors = errors;
  await page.screenshot({ path:"verification-joint-air.png", fullPage:true });
  console.log(JSON.stringify(result,null,2));
  if(errors.length||result.enabled!=="true"||result.total!==6||result.blueLive<1||result.redLive<1||result.launches<5||result.shipSamShots<1||!result.launchLog?.includes("AIM-54A")||!result.launchLog.includes("KSR-5")||!result.launchLog.includes("AGM-84A")||!result.states||!result.phases)process.exitCode=1;
