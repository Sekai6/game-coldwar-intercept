...
try {
  ...
  if(errors.length||result.enabled!=="true"||result.total!==6||result.blueLive<1||result.launches<5||result.shipSamShots<2||result.legacyAirRegistrations!==0||result.legacyAirFields!==0||result.missingEntityRefs!==0||result.ambiguousKindFields!==0||!/MK 10|MK 41/.test(result.airDefenseLaunchers)||!result.airDefenseCategories.includes("aircraft")||!result.airDefenseCategories.includes("missile")||result.aircraftTracks<1||result.weaponTracks<1||!result.launchLog?.includes("AIM-54A")||!result.launchLog.includes("KSR-5")||!result.launchLog.includes("AGM-84A")||!result.missions?.includes("egress")||result.aarAircraft!==6||result.aarWeapons<5||!result.states||!result.formations||!result.phases)process.exitCode=1;
