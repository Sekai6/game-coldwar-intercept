...
async function capture(path, ultra) {
  ...
  try {
    ...
    if (ultra) await page.waitForFunction(() => Number(document.querySelector("#scene")?.dataset.webGpuUltraHistoryFrames ?? 0) >= 12, null, { timeout: 5_000 });
...
await capture("verification-ultra-taa-ship-default.png", true);
await capture("verification-high-ship-default.png", false);
